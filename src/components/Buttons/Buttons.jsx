...
const Buttons = () => {
  ...
  return (
    <div className='h-[70px] w-full  rounded-xl mb-3 bg-[#020817] border text-white border-[#1E293B] box-border px-4 py-2 flex items-center'>
      <div className='flex gap-4 overflow-x-auto  scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-300'>
        {buttons.map((button) => (
          <button
            key={button.id}
            onClick={() => navigate(`/${button.route}`)}
            className={`px-4 py-2 border border-[#1E293B] w-auto  font-semibold rounded-md font-lexend ${
